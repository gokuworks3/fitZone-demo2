...
const saveContactLead = (payload) => {
  const currentEntries = JSON.parse(
    window.localStorage.getItem('fitzoneContactLeads') || '[]',
  )
  ...
  window.localStorage.setItem('fitzoneContactLeads', JSON.stringify(updatedEntries))
}
...
const Contact = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      saveContactLead(formState)
      setStatus({
        type: 'success',
        message: 'Message saved in demo mode. We will contact you soon.',
      })
      setFormState(initialContactForm)
    } catch {
      setStatus({
        type: 'error',
        message: 'Unable to save your message in this browser session.',
      })
    } finally {
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          ...
          >
            ...
            <p className="text-center text-xs uppercase tracking-[0.1em] text-brand-muted">
              Demo Mode: messages are saved locally in your browser.
            </p>
          </Motion.form>
